...
/// メモリのサイズ（バイト単位）
// allow(dead_code)とすることで、このファイル中で一度も使われていなくてもコンパイラが警告を出さないようにする
#[allow(dead_code)]
pub const MEMORY_SIZE: usize = 1024 * 1024 * 128; // 128MB
...
impl Memory {
    ...
    pub fn write_word(&mut self, address: MemoryAddress, value: Word) -> Result<(), MemoryError> {
        // saturating_add()はオーバーフローを防ぎ、結果が型の最大値や最小値を超えそうになった場合、その型の限界値で飽和(saturate)させます。
